...
T_HOLD = const(7)
T_PRESS = const(8)
T_HELD_DURATION = const(9)
NUM_OF_PARAS = const(10)

def decorator_ignore_if_held(func):
    def wrapper(btn: AIOButton):
        # called later with the AIOButton instance
        if btn.para_list[T_HELD_DURATION] < btn.hold_ms:
            return func(btn)
    return wrapper
...
class AIOButton(object):
    ...
    async def coro_check(self):
        while True:
            ...
            if self.para_list[S_DEBOUNCED]:
                self.para_list[T_HELD_DURATION] += self.check_ms
            else:
                self.para_list[T_HELD_DURATION] = 0
